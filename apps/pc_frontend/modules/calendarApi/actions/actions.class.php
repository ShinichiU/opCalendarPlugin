...
class calendarApiActions extends sfActions
{
  ...
  */
  ...
  {
    $tokens = $this->opGoogleCalendarOAuth->getRequestToken();
    $this->getUser()->setAttribute('opGoogleCalendarOAuthTokens', $tokens);

    $this->redirect($this->opGoogleCalendarOAuth->getAuthUrl($tokens['oauth_token']));
  }
  ...
  */
  ...
  {
    $tokens = $this->getUser()->getAttribute('opGoogleCalendarOAuthTokens');
    if (!$tokens)
    {
      $this->getUser()->setFlash('error', '認可処理に失敗しました。もう一度やり直してください');
